...
cal_data$Calanoida[is.na(cal_data$Calanoida)] <- 0

cal_data$logCPUE <- log10((cal_data$Calanoida)+1)
  

#Check to make sure same number of samples
chk_cal_data <- allbugs3%>%
  group_by(Wetland) %>%
  filter(all(c("Inside", "Outside_1k") %in% Type)) %>%
  ungroup()%>%
  mutate(yr_mo=paste(year,month,sep="_"),
  ...
  filter(all(c("Inside", "Outside_1k") %in% Type)) %>%
  ungroup()%>%
  filter(SizeClass=="Meso",year>2016,year<2023,
         site_type!="Planned",
         !Wetland%in%c("Arnold Sl.","Lookout Sl.","Dutch Sl."))%>%
  group_by(SampleID)%>%
    summarise(n=n())

chk_cal_data2 <- allbugs3%>%
  group_by(Wetland) %>%
  filter(all(c("Inside", "Outside_1k") %in% Type)) %>%
  ungroup()%>%
  mutate(yr_mo=paste(year,month,sep="_"),
  ...
  filter(all(c("Inside", "Outside_1k") %in% Type)) %>%
  ungroup()%>%
  filter(Lifestage=="Adult",SizeClass=="Meso",Order=="Calanoida",
         year>2016,year<2023,
         site_type!="Planned",
         !Wetland%in%c("Arnold Sl.","Lookout Sl.","Dutch Sl."))%>%
  group_by(SampleID)%>%
...
unique(cal_data%>%
         select(site_type,Wetland))

calexplore <- cal_data%>%
  group_by(Wetland,Type,year)%>%
  summarise(n=n())%>%
  spread(year,n)
...
m_cal3 <- brm(formula = Calanoida+1 ~ Type*Wetland +
                sea + 
                (1|Source)+
                (1|year),
              data=cal_data,
...
m_cal3.1 <- brm(formula = logCPUE ~ Type*Wetland +
                sea + 
                (1|Source)+
                (1|year),
              data=cal_data,
...
m_clad <- brm(formula = logCPUE ~ Type*Wetland +
                sea + 
                (1|Source)+
                (1|year),
              data=clad_data,
...
m_cyc <- brm(formula = logCPUE ~ Type*Wetland +
                sea + 
                (1|Source)+
                (1|year),
              data=cyc_data,
